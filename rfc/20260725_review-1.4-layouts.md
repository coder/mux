...
The exports measure 1:1 in CSS pixels, so they can be used as a dimensional reference directly:
in `start-workspace-layout.png` the sidebar separator lands at x=271 and the right panel spans
303px, matching the 272px / 303px columns the frames specify.
...
- Under the sentence is a large prompt input with the placeholder "Describe your goals, or leave blank for a codebase summary." That is, submitting with an empty prompt is a valid path that produces a codebase summary.
...
- **Thinking half of the model pill**: `max` is plain text in the model name's color, with no
  badge, no level-coded color ramp, and no stepper chevrons. Clicking it cycles the model's
  allowed levels (the keybinds still step up and down one level).
- **Separators**: the only full-width hairline in the column is under the header; the composer's
  own box border is the sole outline between there and the footer bar. The banners that stack
  above the composer (pinned TODO, queued message, reviews, background bash) therefore render
  without a top divider.
...
The frames are built on the zinc scale plus one cyan accent, and the dark theme already carries
the same values, so the composer references tokens instead of the sampled hex codes:

| Sampled from Figma | Token                              | Used for                                    |
| ------------------ | ---------------------------------- | ------------------------------------------- |
| `#09090b`          | `--color-surface-primary`          | page, chrome, transcript, composer interior |
| `#27272a`          | `--color-border-light` (`#262626`) | composer box and pill borders               |
| `#3f3f46`          | `--color-surface-quaternary`       | context meter track                         |
| `#18181b`          | `--color-surface-secondary`        | send button while sending is unavailable    |
| `#888895`          | `--color-composer-send`            | send button once a message can be sent      |
| `#a1a1aa`          | `--color-content-secondary`        | footer text, meter fill                     |

A few consequences. The prompt has no fill or border of its own: one bordered box holds the
textarea and the pill row, and its interior is the page background. The chrome (left sidebar,
header bars, footer bar) drops its lighter fill, so `--color-sidebar` resolves to
`--color-surface-primary` in the dark theme and hairline borders do the separating; the other
themes keep their own sidebar fill. `--color-composer-send` is a new per-theme token because no
existing neutral carries this value, and the frames encode send-ability in the fill alone: the
glyph stays the page color in both states, so only the circle changes. The three frames that
draw a send button disagree on its fill, so the two workspace frames win over the creation one
(see the send-button deviation below). The Electron titlebar overlay, the `theme-color` meta
tag, and the PWA manifest cannot read CSS variables, so they carry a duplicate palette in
`src/common/constants/chromeColors.ts` that mirrors each theme's `--color-sidebar`.
...
- **Creation sentence icons**: the design's configurator sentence carries no icon between
  "will use" and the runtime select, so the decorative blocks glyph and the adjacent runtime
  settings shortcut were dropped. Project runtime defaults stay reachable from Settings,
  which has its own project scope picker.
- **Send button fill**: the three frames that draw a send button disagree.
  `footer-info-bar.png` (prompt has text) fills it `#888895`, `chat-column.png` (empty prompt)
  fills it `#18181b`, and `start-v2-creation.png` (empty prompt) fills it `#a4e8f2` cyan. The
  two workspace frames are self-consistent as an enabled/disabled pair, so they define the
  states and the cyan creation variant is treated as design iteration noise. The glyph is
  `#09090b` in all three.
...
- **Blank-prompt creation**: the design's creation placeholder advertises "or leave blank for
  a codebase summary", but `canSend` requires text, attachments, or reviews, and no
  codebase-summary behavior exists. The placeholder omits the promise until the feature lands.
- **Creation sentence slot mapping**: the design's sentence is "My project will use [select
  source] with [Select model] with [Select level] thinking." on the zero-projects screen, where
  "source" means choosing a repository. The implemented sentence covers the has-project screen,
  so it states workspace type and source branch (both required by `workspace.create`) and
  leaves model and thinking level in the composer row shared with the chat view. Runtimes that
  need one more value extend the sentence with a clause (`on host ...`, `with image ...`);
  Coder keeps its own row below because its form is several fields wide.
...
- **Drift glyphs**: the design renders drift as `↓12 ↑233`; the app reuses
  ...
  consistent. The design's grouping is matched: both counts share one pill split by a
  divider. The behind-only fallback stays outside the pill because it is a single value.
- **Composer label contrast**: the design sets the model, thinking, and context labels around
  `#52525b` on `#09090b`, roughly 2.5:1, which fails WCAG AA. The app keeps its readable
  foreground and muted greys, so the pill row reads brighter than the frames.
- **Amber configurator pills**: the design outlines the creation sentence's three selects in
  amber (`#fcba74`). The app keeps per-runtime colors (worktree purple, Docker blue, and so on)
  because they identify the runtime, and neutral hairlines for the rest.
- **Agent pill hue**: the design's pill reads `Auto: Plan` in cyan; the app tints the label and
  icon with the active agent's `uiColor` (blue for plan, purple for exec, custom agents supply
  their own).
- **Send-mode chevron**: the design pairs the circular send button with a chevron dropdown.
  #2633 deliberately removed that caret to keep send single-action, moving advanced dispatch
  modes to right-click / long-press plus `Enter` / `Ctrl+Enter`. `tests/ui/chat/sendModeDropdown.test.ts`
  guards the absence, so restoring the chevron is a product reversal, not a layout fix.
- **`Auto: <mode>` agent label**: the design's mode selector has an auto-select toggle, so its
  pill distinguishes `Auto: Plan` from `Planning`. The app resolves `auto` to a concrete agent
  before render and has no auto-select state to expose, so the pill always shows the agent name.
- **Footer slash separators**: the design separates footer items with `/`. The bar's items are
  individually conditional (PR, drift, repo, branch, usage, last prompt all drop out), so the
  app spaces them with `gap-2` instead of maintaining separator placement per combination.
- **Footer overflow affordance**: the design pins an ellipsis/arrow control at the right edge
  when the bar is too long. The bar scrolls horizontally instead, which satisfies the design
  note's "slide to the remaining info" without adding a control.
- **Context meter segment colors**: the palette table maps meter fill to
  `--color-content-secondary`, and the frames show a neutral grey meter. The app keeps
  `TokenMeter`'s per-category colors (cache create, input, output) because the hues carry
  information the flat fill would discard.
- **Footer text token**: the palette table maps footer text to `--color-content-secondary`, but
  the bar's shared children own their own greys (`BranchSelector` hardcodes `text-muted-light`,
  `GitStatusIndicatorView` uses `text-muted`). Those two sit close enough to read as one strip,
  while `--color-content-secondary` is far enough from both to split it visibly. The footer-local
  labels therefore keep `text-muted` until those components accept a color variant.
