...
| `#a4e8f2`          | `--color-composer-send`            | send button once a message can be sent      |
...
themes keep their own sidebar fill. `--color-composer-send` is a new per-theme token because the
design's cyan is not `--color-accent`'s blue, and `--color-border-pending` (the same cyan today)
means "needs attention". The Electron titlebar overlay, the `theme-color` meta tag, and the PWA
manifest cannot read CSS variables, so they carry a duplicate palette in
