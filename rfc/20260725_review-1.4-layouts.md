...
- **Footer text token**: the palette table maps footer text to `--color-content-secondary`, but
  ...
  `GitStatusIndicatorView` uses `text-muted`). Moving only the footer-local labels splits the
  strip into two visibly different greys, so the bar stays uniformly `text-muted` until those
  components take a color variant.
