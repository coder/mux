...
means "needs attention". The Electron titlebar overlay and the `theme-color` meta tag cannot read
CSS variables, so they carry a duplicate palette in `src/common/constants/chromeColors.ts` that
mirrors each theme's `--color-sidebar`.
