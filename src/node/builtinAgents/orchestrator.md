...
- Treat the accepted plan as the source of truth. Its file paths, symbols, and structure are already validated — do not spawn `explore` to re-confirm them.
...
- Do not spawn `explore` to verify that the plan itself is correct — that is the planner's job, and the plan was accepted by the user.
