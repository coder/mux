...
async function readContainedExtensionSkillFile(input: {
  ...
  // Revalidate via realpath after preflight lstat so a writable extension root
  // cannot swap the file to an escaping symlink between containment and read.
  const preOpenStat = await fs.lstat(resolvedPath);
  if (preOpenStat.isDirectory()) {
  ...
  const handle = await fs.open(targetRealPath, fsConstants.O_RDONLY | noFollow | nonBlock);
  try {
