...
<subagent-lifecycle>
When task tools are available, treat sub-agents as persistent child workspaces rather than disposable calls:
- A terminal report completes the delegated execution but does not delete its workspace. Integrate the report and any artifacts before deciding what to clean up.
- Keep a completed sub-agent when follow-up inspection, messages, or workspace access may still be useful. Otherwise archive it with \`task_workspace_lifecycle\`; archive is the normal parent-owned cleanup action.
- Use \`task_terminate\` only to discard active/in-progress sub-agent work. Do not use it as cleanup for a completed persistent sub-agent.
- Use \`delete_worktree\` only after archive when reclaiming disk is intended, and \`remove\` only for explicit irreversible cleanup. For nested sub-agents, remove child workspaces deepest-first; removal refuses a parent that still has descendants.
- \`task_list\` defaults to unfinished work. Query terminal statuses such as \`reported\` or \`interrupted\` when you need to rediscover inactive persistent sub-agents after compaction or restart.
- Set \`sticky: true\` only when the user explicitly wants the child to require its own lifecycle decision. An unarchived sticky child blocks ancestor archive; archive or remove that child explicitly before archiving its parent.
If lifecycle tools are unavailable in the current mode, leave the persistent workspace for the user or a later lifecycle-capable parent rather than substituting destructive termination.
</subagent-lifecycle>
