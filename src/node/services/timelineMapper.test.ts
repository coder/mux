...
describe("mapChatEventToTimeline", () => {
  test("discriminates compaction and reset boundary messages", () => {
    ...
    expect(compaction.drafts).toEqual([
      expect.objectContaining({ kind: "compaction.completed", epoch: 2, ts: 100 }),
    ]);
    expect(reset.drafts).toEqual([expect.objectContaining({ kind: "context.reset", ts: 101 })]);
  });
  ...
  test("maps stream completion and failure lifecycle events", () => {
    ...
    expect(completed.drafts).toEqual([
      expect.objectContaining({
        ...
        data: expect.objectContaining({ model: "claude-sonnet-4", durationMs: 75 }),
      }),
    ]);
  ...
  test("synthesizes interrupted rows for notable open tools on stream abort", () => {
    ...
    expect(aborted.drafts).toEqual([
      expect.objectContaining({ kind: "turn.interrupted", status: "interrupted" }),
      expect.objectContaining({
        kind: "tool.call",
        status: "interrupted",
        anchor: expect.objectContaining({ toolCallId: "tool-1" }),
        data: expect.objectContaining({ toolName: "file_edit_insert", digest: "src/app.ts" }),
      }),
    ]);
