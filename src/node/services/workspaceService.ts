...
export class WorkspaceService extends EventEmitter {
  ...
  > {
    try {
      ...
      const namedWorkspacePath = runtime.getWorkspacePath(projectPath, branchName);
  ...
  async replaceHistory(workspaceId: string, summaryMessage: MuxMessage): Promise<Result<void>> {
    ...
    try {
      ...
      if (session) {
        session.emitChatEvent(summaryMessage);
      } else {
        this.emit("chat", { workspaceId, message: summaryMessage });
      }
