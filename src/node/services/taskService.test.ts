...
describe("TaskService", () => {
  ...
  test("does not suppress agent_report reminder due to historical compaction summary", async () => {
    const config = await createTestConfig(rootDir);
    ...
    const followUpAssistant = createMuxMessage(
      "assistant-follow-up",
      "assistant",
      "Follow-up response",
      { timestamp: Date.now() }
    ...
    expect((await historyService.appendToHistory(childId, followUpAssistant)).success).toBe(true);
  ...
  test("skips agent_report reminder on compaction stream-end while follow-up is in progress", async () => {
    const config = await createTestConfig(rootDir);
    ...
    const followUpAssistantPlaceholder = createMuxMessage("assistant-follow-up", "assistant", "", {
      timestamp: Date.now(),
    });
    ...
    expect((await historyService.appendToHistory(childId, followUpUser)).success).toBe(true);
    expect(
      (await historyService.appendToHistory(childId, followUpAssistantPlaceholder)).success
    ).toBe(true);
    ...
    await internal.handleStreamEnd({
      type: "stream-end",
      workspaceId: childId,
      messageId: "assistant-compaction-output",
      metadata: { model: "openai:gpt-4o-mini" },
      parts: [],
    });
