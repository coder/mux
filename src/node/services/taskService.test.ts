...
describe("TaskService", () => {
  ...
  test("stream-abort after agent_report finalizes report and triggers cleanup", async () => {
    const config = await createTestConfig(rootDir);
    ...
    const childReportMessage = createMuxMessage(
    ...
    const appendChildHistory = await historyService.appendToHistory(childId, childReportMessage);
    ...
    await internal.handleStreamAbort({
      type: "stream-abort",
      workspaceId: childId,
      messageId: childReportMessage.id,
    });
  ...
  test("stream-abort does not finalize from stale messageId", async () => {
    const config = await createTestConfig(rootDir);
    ...
    const oldChildReportMessage = createMuxMessage(
      "assistant-child-old-report",
      "assistant",
      "old report",
      { timestamp: Date.now() },
      [
        {
          type: "dynamic-tool",
          toolCallId: "agent-report-call-old",
          toolName: "agent_report",
          input: { reportMarkdown: "Old report", title: "Old" },
          state: "output-available",
          output: { success: true },
        },
      ]
    );
    const appendOldHistory = await historyService.appendToHistory(childId, oldChildReportMessage);
    expect(appendOldHistory.success).toBe(true);
    ...
    await internal.handleStreamAbort({
      type: "stream-abort",
      workspaceId: childId,
      messageId: "assistant-child-newer-message",
    });
    ...
    const unchangedParentPartial = await partialService.readPartial(parentId);
    expect(unchangedParentPartial).not.toBeNull();
    if (unchangedParentPartial) {
      const toolPart = unchangedParentPartial.parts.find(
        (p) =>
          p &&
          typeof p === "object" &&
          "type" in p &&
