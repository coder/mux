...
describe("StreamManager - empty stream completions", () => {
  ...
  async function processProviderStreamForCompletionTest(params: {
    workspaceId: string;
    messageId: string;
    model?: string;
    stream: AsyncGenerator<unknown, void, unknown>;
  }): Promise<{
    errorEvents: Array<{ messageId: string; error: string; errorType?: string }>;
    streamEndEvents: unknown[];
  }> {
    const streamManager = new StreamManager(historyService);
    ...
    const model = params.model ?? KNOWN_MODELS.SONNET.id;
    const streamInfo = createStreamInfoForTests({
      streamResult: createStreamResultForTests(params.stream, {
        inputTokens: 3,
        outputTokens: 2,
        totalTokens: 5,
      }),
    ...
    return { errorEvents, streamEndEvents };
  }
  ...
  test("treats synthesized other finish without raw reason as stream_truncated", async () => {
    const workspaceId = "openai-synthesized-other-workspace";
    const messageId = "openai-synthesized-other-message";

    const { errorEvents, streamEndEvents } = await processProviderStreamForCompletionTest({
      workspaceId,
      messageId,
      model: "openai:gpt-4.1-mini",
    ...
    expect(errorEvents[0]?.error).toContain(
      "OpenAI stream closed unexpectedly before the response completed"
    );
  ...
  test("allows legitimate other finish when a raw reason is present", async () => {
    const workspaceId = "openai-legitimate-other-workspace";
    const messageId = "openai-legitimate-other-message";
    ...
    const { errorEvents, streamEndEvents } = await processProviderStreamForCompletionTest({
      ...
      stream: (async function* () {
        ...
        yield {
          type: "finish",
          finishReason: "other",
          rawFinishReason: "safety_violation",
        };
      })(),
    });
    ...
    expect(updatedMessage?.metadata?.finishReason).toBe("other");
    expect(updatedMessage?.parts).toMatchObject([{ type: "text", text: "complete answer" }]);
  });
});
