...
type SSHRuntimeConstructor = new (...args: any[]) => SSHRuntime;
 ...
 */
function createCoderSSHRuntime<TBase extends SSHRuntimeConstructor>(Base: TBase) {
  return class CoderSSHRuntimeBase extends Base {
    ...
    constructor(...args: any[]) {
      const [config, coderService] = args as [CoderSSHRuntimeConfig, CoderService];
      if (!config || !coderService) {
        throw new Error("CoderSSHRuntime requires config and coderService");
      }
    ...
    private async doEnsureReady(
      ...
      const remainingMs = () =>
        Math.max(0, CODER_ENSURE_READY_TIMEOUT_MS - (Date.now() - startTime));
...
const CoderSSHRuntimeBase = createCoderSSHRuntime(SSHRuntime as SSHRuntimeConstructor);
export class CoderSSHRuntime extends CoderSSHRuntimeBase {
  constructor(config: CoderSSHRuntimeConfig, coderService: CoderService) {
    super(config, coderService);
  }
}

const CoderSSH2RuntimeBase = createCoderSSHRuntime(SSH2Runtime as SSHRuntimeConstructor);
export class CoderSSH2Runtime extends CoderSSH2RuntimeBase {
  constructor(config: CoderSSHRuntimeConfig, coderService: CoderService) {
    super(config, coderService);
  }
}
