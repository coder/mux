...
import { filterEffectPermissions } from "@/common/extensions/permissionCalculator";
...
function approvalCoversEffectPermissions(
  approval: ApprovalRecord | undefined,
  effectPermissions: readonly string[]
): boolean {
  if (effectPermissions.length === 0) return true;
  if (!approval) return false;
  const granted = new Set(approval.grantedPermissions);
  return Array.from(new Set(effectPermissions)).every((permission) => granted.has(permission));
}
...
async function extractContainedStaticManifest(input: {
  ...
  try {
    handle = await fsPromises.open(entrypointRealPath, fsConstants.O_RDONLY | noFollow | nonBlock);
  } catch (error) {
    return {
      ok: false,
      diagnostics: [
...
async function discoverCandidateExtensionModule(
  ...
  const effectPermissions = filterEffectPermissions(
    manifest.requestedPermissions,
    registrationRequestedPermissions
  );
  const granted = isBundled || approvalCoversEffectPermissions(approvalRecord, effectPermissions);
  const approvedForCurrentPermissions = granted;
...
async function readActivationBodyFile(realPath: string): Promise<{
  ...
  const preOpenStat = await fsPromises.lstat(realPath);
  if (!preOpenStat.isFile()) {
    return { content: null, sizeValidationError: "Path is not a regular file." };
  }

  const noFollow = fsConstants.O_NOFOLLOW ?? 0;
  const nonBlock = fsConstants.O_NONBLOCK ?? 0;
  const handle = await fsPromises.open(realPath, fsConstants.O_RDONLY | noFollow | nonBlock);
  try {
