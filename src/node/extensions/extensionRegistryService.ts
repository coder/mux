  ...
  async setApproval(scope: ExtensionScope, extensionId: string): Promise<void> {
    return this.enqueue(async () => {
      ...
      const effectPermissions = filterEffectPermissions(
        ...
        liveApprovalMaterial.registrationPermissions
      );
      const normalized: ApprovalRecord = {
        grantedPermissions: effectPermissions,
        requestedPermissionsHash: hashRequestedPermissions(effectPermissions),
      };
      if (scope.kind === "global") {
    ...
    for (const root of snapshot.roots) {
      for (const ext of root.extensions) {
        ...
        return {
          ...
          registrationPermissions: registrationPermissionsForManifest(ext.manifest),
        };
      }
    }
    return undefined;
...
function registrationPermissionsForManifest(manifest: DiscoveredExtension["manifest"]): string[] {
  return Array.from(
    new Set(
      manifest.contributions.map(
        (contribution) =>
          CONTRIBUTION_TYPE_REGISTRATION_PERMISSIONS[contribution.type] ??
          `${contribution.type}.register`
      )
    )
  );
}
...
function synthesizePolicyApproval(ext: DiscoveredExtension): ApprovalRecord {
  const effectPermissions = filterEffectPermissions(
    ext.manifest.requestedPermissions,
    registrationPermissionsForManifest(ext.manifest)
  );
  return {
    grantedPermissions: effectPermissions,
    requestedPermissionsHash: hashRequestedPermissions(effectPermissions),
