...
export class ExtensionRootWatcher {
  ...
  private onModuleWatchEvent(filename: string | null): void {
    // Module directory watchers exist so editing <module>/extension.ts or any
    // contained skill body path triggers rediscovery even where root fs.watch is not recursive.
...
async function findExtensionModuleWatchPaths(rootPath: string): Promise<string[]> {
  ...
  for (const entry of entries) {
    ...
    const skillsPath = path.join(modulePath, "skills");
    if (await directoryExists(skillsPath)) {
      watchPaths.push(skillsPath);
      watchPaths.push(...(await findNestedDirectoryPaths(skillsPath)));
    }
  }
  return watchPaths;
}
