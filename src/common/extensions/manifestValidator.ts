...
export const RESERVED_EXTENSION_NAME_PREFIX_REGEX = /^mux(?:-.+)?$/;
...
export function validateStaticManifest(
  ...
  if (
    ...
    RESERVED_EXTENSION_NAME_PREFIX_REGEX.test(input.extensionName)
  ) {
    diagnostics.push({
      code: "extension.identity.reserved",
      severity: "error",
      message: `Extension Name ${JSON.stringify(input.extensionName)} uses the reserved first-party namespace.`,
      extensionId: input.extensionName,
      occurredAt,
    });
    return { ok: false, diagnostics };
