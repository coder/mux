...
describe("validateStaticManifest", () => {
  ...
  test("rejects non-bundled static manifests using the reserved mux name prefix", () => {
    const result = validateStaticManifest({
      rawManifest: { name: "mux-platform-demo", capabilities: { skills: true } },
      extensionName: "mux-platform-demo",
      rootKind: "user-global",
      now: FROZEN_NOW,
    });
  ...
  test("accepts bundled static manifests using the reserved mux name prefix", () => {
    const result = validateStaticManifest({
      rawManifest: { name: "mux-platform-demo", capabilities: { skills: true } },
      extensionName: "mux-platform-demo",
      rootKind: "bundled",
    ...
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.manifest.id).toBe("mux-platform-demo");
  });
