 ...
 * model-visible tool list until the model discovers them via the `tool_search`
...
/** Default / max number of matches returned by a tool_search call. */
 ...
 * filtering, mutated by `tool_search.execute` (activations) and by the
...
export interface ToolSearchStreamState {
  ...
  allToolNames: string[];
  /** Deferred tools discovered via tool_search (or prior-turn history). */
 ...
 * `tool_search` itself. Intersection semantics absorb policy-disable and
 ...
 *   `tool_search` from the record (a search tool with an empty catalog is
 ...
 * `tool_search` is removed from the returned record.
 */
...
/** Read `matches[].name` strings from a defensively-parsed tool_search result value. */
...
/**
 * Seed the activation set from prior tool_search results found in the
