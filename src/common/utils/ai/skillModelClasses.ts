...
export function parseModelClassValue(value: string): ModelClassTarget | null {
  ...
  const modelPart = plusIndex === -1 ? trimmed : trimmed.slice(0, plusIndex);
  const thinkingPart = plusIndex === -1 ? null : trimmed.slice(plusIndex + 1);
...
/**
 * Split a class value into its model part and raw thinking suffix. The raw
 * suffix is what follows the first "+" verbatim: editors preserve it across
 * model changes so a model-relative numeric level ("+0" = lowest allowed)
 * keeps its meaning on the new model.
 */
export function splitModelClassValue(value: string): {
  ...
  return { modelPart: value.slice(0, plusIndex), thinkingSuffix: value.slice(plusIndex + 1) };
}
