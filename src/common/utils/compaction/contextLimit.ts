...
export function getEffectiveContextLimit(
  ...
  // 1M context is a runtime model capability (e.g., Anthropic beta header,
  // OpenAI GPT-5.4 extended context mode), so it must be gated on the runtime
  // model, not the mapped metadata model.
