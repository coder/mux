...
type InspectionDescriptor = z.infer<typeof schemas.InspectionDescriptorSchema>;
...
function makeDescriptor(overrides: Partial<InspectionDescriptor> = {}): InspectionDescriptor {
  return {
    type: overrides.type ?? "skills",
    id: overrides.id ?? "demo.skill",
    extensionId: overrides.extensionId ?? "vendor.demo",
    rootId: overrides.rootId ?? "root-1",
    rootKind: overrides.rootKind ?? "user-global",
...
describe("ExtensionCard", () => {
  ...
  test("body-failed skill contributions render as unavailable", () => {
    const base = makeExtension();
    ...
    const view = renderCard(
      ...
      false,
      {},
      [makeDescriptor({ available: false, unavailableReasons: ["body-failed"] })]
    );
  ...
  test("extension identity conflicts render losing skill contributions as unavailable", () => {
    const ext = makeExtension({
      diagnostics: [diag({ code: "extension.identity.conflict", severity: "error" })],
    });
    const view = renderCard(ext, makePermissions(), false, {}, [
      makeDescriptor({ available: false, unavailableReasons: ["conflict"] }),
    ]);
  ...
  test("extension identity conflicts preserve winning skill availability", () => {
    const ext = makeExtension({
      diagnostics: [diag({ code: "extension.identity.conflict", severity: "error" })],
    });
    const view = renderCard(ext, makePermissions(), false, {}, [
      makeDescriptor({ available: true }),
    ]);
  ...
  test("conflicted skill contributions render as unavailable", () => {
    const ext = makeExtension({
      diagnostics: [
        diag({
          code: "contribution.identity.conflict",
