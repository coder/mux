        ...
        <div id={`extensions-root-${rootDomId}`} className="space-y-2 px-3 py-3">
          ...
          {extensions.length > 0 && (
            <div className="space-y-2">
              ...
              {extensions.map((ext) => {
                const extensionDescriptors = descriptors.filter(
                  (descriptor) =>
                    descriptor.extensionId === ext.extensionId && descriptor.rootId === ext.rootId
                );
                return (
                  <ExtensionCard
...
export const ExtensionsSection: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        if (!abort.signal.aborted) await refresh();
  ...
  const handleConsentConfirm = useCallback(async () => {
    ...
    try {
      ...
      // Do not call untrustRoot here: that endpoint also clears project-wide trust,
      // so a failed extension setup must not revoke unrelated project trust.
