...
export async function subscribeToExtensionSnapshots(input: {
  api: APIClient;
  signal: AbortSignal;
  onSnapshot: (snapshot: RegistrySnapshot | null) => void;
}): Promise<void> {
  const refresh = async () => {
    try {
      input.onSnapshot((await input.api.extensions.list()) ?? null);
    } catch {
      /* expected on shutdown */
    }
  };
  const iter = await input.api.extensions.onChanged(undefined, { signal: input.signal });
  if (!input.signal.aborted) await refresh();
