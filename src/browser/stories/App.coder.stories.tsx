...
/**
 * Coder CLI outdated - Coder button should not appear.
 * When Coder CLI version is below minimum, the button is hidden (same as unavailable).
 */
export const CoderOutdated: AppStory = {
  ...
  play: async ({ canvasElement }: { canvasElement: HTMLElement }) => {
    ...
    // Coder button should NOT appear when CLI is outdated
    const coderButton = canvas.queryByRole("button", { name: /Coder/i });
    if (coderButton) {
      throw new Error("Coder button should not appear when CLI is outdated");
    }
  },
};
