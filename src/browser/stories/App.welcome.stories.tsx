...
/**
 * Workspace name validation error - shows error toast when workspace name contains invalid characters.
...
export const WorkspaceNameValidationError: AppStory = {
  ...
  play: async ({ canvasElement }: { canvasElement: HTMLElement }) => {
    ...
    // Wait for creation view to be ready
    const input = await canvas.findByPlaceholderText(
      /workspace-name|Generating/i,
      {},
      { timeout: 10000 }
    );

    // Type an invalid workspace name and message
    await userEvent.clear(input);
    await userEvent.type(input, "Invalid/Name");

    // Type a message and submit to trigger creation
