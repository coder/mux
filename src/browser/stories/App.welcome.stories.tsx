...
/**
 * Workspace name validation error - shows error toast when workspace creation fails.
 * Tests the solid background color fix (no transparency bleed with underlying elements).
 */
export const WorkspaceNameValidationError: AppStory = {
  parameters: {
    chromatic: {
      modes: {
        dark: { theme: "dark" },
  ...
  play: async ({ canvasElement }: { canvasElement: HTMLElement }) => {
    ...
    // Wait for creation view to be ready - the input may show "Generating..." placeholder initially
    await canvas.findByPlaceholderText(/workspace-name|Generating/i, {}, { timeout: 10000 });

    // Type a message and submit to trigger creation (name doesn't matter since mock always fails)
    ...
    await waitFor(
      async () => {
        // Toast appears in the storyRoot with role="alert"
        const toast = storyRoot.querySelector('[role="alert"]');
        if (!toast) {
          throw new Error("Error toast not visible");
        }
        // Verify it contains our error message
        await expect(toast.textContent).toContain("lowercase");
      },
      { timeout: 5000 }
    );
  },
