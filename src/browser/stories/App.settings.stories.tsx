...
export const System1: AppStory = {
  ...
  play: async ({ canvasElement }: { canvasElement: HTMLElement }) => {
    ...
    await settingsCanvas.findByRole("heading", { name: /memories/i });
    await settingsCanvas.findByText(/Write Interval \(messages\)/i);
    ...
    await waitFor(() => {
      const inputs = settingsCanvas.queryAllByRole("spinbutton");
      if (inputs.length !== 5) {
        throw new Error(`Expected 5 System 1 inputs, got ${inputs.length}`);
      }

      const values = inputs.map((input) => (input as HTMLInputElement).value);
      const expectedValues = ["12", "8", "55", "9"];
      for (const expected of expectedValues) {
        if (!values.includes(expected)) {
          throw new Error(
            `Expected System 1 numeric controls to include ${JSON.stringify(expected)}, got ${JSON.stringify(values)}`
          );
        }
      }
    });
