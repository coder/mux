...
import { RESERVED_EXTENSION_NAME_PREFIX_REGEX } from "@/common/extensions/manifestValidator";
...
async function createLocalExtensionModule(input: {
  ...
  if (RESERVED_EXTENSION_NAME_PREFIX_REGEX.test(parsedName.data)) {
    throw new Error(
      `Extension Name ${JSON.stringify(parsedName.data)} uses the reserved first-party namespace.`
    );
  }
