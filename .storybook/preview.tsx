...
// Signal Storybook runtime to modules that need to stabilize for snapshots
...
function ensureStorybookFontsReady(): Promise<void> {
  fontsReadyPromise ??= (async () => {
    ...
    // Trigger load of layout-affecting fonts so stories don't snapshot mid font-swap.
...
const preview: Preview = {
  ...
  decorators: [
    // Theme provider
    (Story, context) => {
      // Default to dark if mode not set (e.g., a headless browser defaults to light)
      const mode = (context.globals.theme as ThemeMode | undefined) ?? "dark";

      // Apply theme synchronously before React renders - critical for stable snapshots
      if (typeof document !== "undefined") {
