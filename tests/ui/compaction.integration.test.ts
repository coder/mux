 ...
 *
 * These tests run with AIService mock mode (MUX_MOCK_AI=1) in router mode.
 */
...
describe("Compaction UI (mock AI router)", () => {
  ...
  const previousMockAiMode = process.env.MUX_MOCK_AI_MODE;

  beforeAll(async () => {
    process.env.MUX_MOCK_AI = "1";
    process.env.MUX_MOCK_AI_MODE = "router";
  ...
  afterAll(() => {
    ...
    if (previousMockAiMode === undefined) {
      delete process.env.MUX_MOCK_AI_MODE;
    } else {
      process.env.MUX_MOCK_AI_MODE = previousMockAiMode;
    }
  });
