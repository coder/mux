...
// TODO: This test requires a debug IPC method (triggerStreamError) that needs to be exposed via ORPC
// Skipping until debug methods are added to ORPC router
const describeSkip = describe.skip;
describeSkip("Stream Error Recovery (No Amnesia)", () => {
...
IMPORTANT: Do not add any other text. Start immediately with ${nonce}-1: one. If interrupted, resume from where you stopped without repeating any lines.`;
        ...
        // NOTE: triggerStreamError is a debug method that needs to be added to ORPC router
        // For now, skip this test - see describe.skip above
        throw new Error("triggerStreamError method not available in ORPC - test skipped");
