...
export class StreamCollector {
  ...
  /**
   * Wait for the ORPC subscription to be established.
  ...
  private async collectLoop(): Promise<void> {
    try {
      ...
      // Note: The generator body (including onChatEvent subscription) doesn't run until
      // we start iterating. We need to pull at least one value to ensure the subscription
      // is established, then mark as ready.
      ...
      for await (const message of iterator) {
        ...
        // After receiving the first event, the subscription is definitely established
      ...
      // If we never received any events, still signal ready to prevent hangs
      if (!firstEventReceived) {
        this.subscriptionReady = true;
        if (this.subscriptionReadyResolve) {
          this.subscriptionReadyResolve();
