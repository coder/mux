 ...
 * Format: "cmux.workspace.{sanitizedWorkspaceId}.{setting}"
 */
function getWorkspaceSettingKey(workspaceId: string, setting: string): string {
  return `cmux.workspace.${sanitizeWorkspaceId(workspaceId)}.${setting}`;
}

/**
 * Get the storage key for a global default setting
 * Format: "cmux.defaults.{setting}"
 */
function getDefaultSettingKey(setting: string): string {
  return `cmux.defaults.${setting}`;
}

/**
 * Get the legacy global default key (for migration)
 * Format: "cmux.workspace-defaults.{setting}"
 */
function getLegacyDefaultKey(setting: string): string {
  return `cmux.workspace-defaults.${setting}`;
}
 ...
 * 2. Global default
 * 3. Legacy global default (for migration)
 * 4. Hardcoded default
 */
async function readSetting<T>(
  ...
  try {
    ...
    // Tier 2.5: Check legacy key for migration
    const legacyKey = getLegacyDefaultKey(setting);
    const legacyValue = await SecureStore.getItemAsync(legacyKey);
    if (legacyValue !== null) {
      if (validator) {
        const validated = validator(legacyValue);
        if (validated !== null) {
          // Migrate legacy value to new default key
          await SecureStore.setItemAsync(defaultKey, legacyValue);
          return validated;
        }
      } else {
        // Migrate legacy value to new default key
