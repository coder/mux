 ...
 *
 * Xum is an anagram of Mux, so the outlined paths reuse the exact Geist Sans Bold
 * glyphs from the prior Mux logo in x-u-m order. This preserves the established
 * baseline, scale, and cursor spacing without relying on platform font rendering.
 */

const XUM_MARK_PATH =
  "M13.125 47.5 20.9101 36.4133 13.3288 25.6526H19.8096L24.497 32.6634L29.0214 25.6526H35.6652L28.1246 36.454L35.869 47.5H29.3882L24.5785 40.0817L19.7281 47.5H13.125Z";
const XUM_MARK_CURSOR_X = 38.875;
const XUM_MARK_CURSOR_Y = 24.5;
const XUM_MARK_CURSOR_WIDTH = 20;
const XUM_MARK_CURSOR_HEIGHT = 23;
...
<!-- Reuse the prior Mux x glyph at the established square-mark scale. -->
<path d="${XUM_MARK_PATH}" fill="${fill}"/>
<rect x="${XUM_MARK_CURSOR_X}" y="${XUM_MARK_CURSOR_Y}" width="${XUM_MARK_CURSOR_WIDTH}" height="${XUM_MARK_CURSOR_HEIGHT}" fill="${fill}"/>
</svg>
`;
}
