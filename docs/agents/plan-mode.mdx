...
```mermaid
graph TD
  A["User request"] --> B["Agent reads codebase\nwrites plan file"]
  B --> C["Agent calls propose_plan\n(plan displayed in chat)"]
  C --> D{"Review + iterate"}
  D --> E["User feedback in chat"]
  D --> F["User clicks Edit\nedits plan in editor"]
  F --> G["User sends message\nmux detects changes\ninjects diff"]
  E --> H["Agent revises plan\n(cycles back to propose_plan)"]
  G --> H
  H --> I["User switches to Exec mode"]
  I --> J["Agent implements the plan"]
```
