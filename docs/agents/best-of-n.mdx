...
If you want sibling sub-agents to cover **different named slices** — for example `frontend`,
`backend`, `tests`, or a list of specific commits — use the task tool's `variants` parameter instead.
When you do, include `${variant}` in the prompt so each child receives its own substituted label.
