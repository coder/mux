...
import ToolHooksExperimental from "/snippets/tool-hooks-experimental.mdx";

<ToolHooksExperimental />
...
This applies to `tool_pre`, `tool_post`, `tool_env`, and the legacy `tool_hook`.

For SSH workspaces, hooks execute **on the remote machine**.

**Priority:** `tool_pre`/`tool_post` take precedence over the legacy `tool_hook` when both exist.
...
<Accordion title="Legacy: tool_hook">

The original `tool_hook` mechanism uses a marker protocol where a single hook handles both pre and post logic:

```bash
#!/usr/bin/env bash
# Pre-hook logic here

echo "$MUX_EXEC"        # Signal: run the tool
read -t 300 MUX_RESULT  # Read tool result from stdin

# Post-hook logic here
```

This still works but `tool_pre` + `tool_post` are simpler—no protocol to learn. If both exist, `tool_pre`/`tool_post` take precedence.
