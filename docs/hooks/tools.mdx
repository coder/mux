...
- **Pre-hook timeout**: the hook must print `__MUX_EXEC__` within 5 minutes (otherwise the tool is not run)
...
The agent always sees hook output (stdout after `__MUX_EXEC__`, plus stderr) in the `hook_output` field of the tool result. This lets you communicate what the hook did—for example, if a formatter modified the file:
