...
1. `.mux/tool_pre` or `.mux/tool_post` (project-level)
2. `~/.mux/tool_pre` or `~/.mux/tool_post` (user-level)

For SSH workspaces, hooks execute **on the remote machine**.

**Priority:** If both `tool_pre` and the legacy `tool_hook` exist, `tool_pre` takes precedence.
