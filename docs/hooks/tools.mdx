...
# Load direnv/devenv environment so tools see correct PATH, env vars, etc.
# This is critical for bash tools that need nvm, pyenv, virtualenvs, etc.
...
if [[ "$MUX_TOOL" == file_edit_* ]]; then
  # Read tool input (may be in a file if large)
  ...
  if [[ "$file" == *.py ]]; then
    ...
    # Check for errors (non-zero exit feeds back to agent)
