...
Our security architecture considers the remote machine potentially hostile. No keys or credentials are implicitly transferred there—just the git archive and [Project Secrets](/config/project-secrets).
...
- **Security**: Prompt injection risk is contained to the credentials / files on the remote machine.
  - SSH remotes pair nicely with [agentic git identities](/config/agentic-git-identity)
