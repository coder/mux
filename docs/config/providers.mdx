...
| `baseUrl`      | Yes      | The OpenAI-compatible API base URL. Xum appends `/v1` when the URL has no path.       |
...
`http://localhost:8080`, Xum appends `/v1`; explicit paths remain unchanged. To configure a
