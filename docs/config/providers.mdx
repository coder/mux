...
| **OrcaRouter**     | 300+ models, one key        | [orcarouter.ai](https://www.orcarouter.ai/)             |
...
[OrcaRouter](https://www.orcarouter.ai/) is an OpenAI-compatible gateway that exposes
300+ models from leading providers through a single API key. Configure it like any other
gateway provider:
...
```jsonc
{
  "orcarouter": {
    "apiKey": "sk-orca-...",
    // Optional: override the default endpoint (https://api.orcarouter.ai/v1)
    // "baseUrl": "https://api.orcarouter.ai/v1",
  },
}
```

Models are addressed with the same `<vendor>/<model>` format as OpenRouter
(e.g. `openai:gpt-5` routes to `orcarouter:openai/gpt-5`).
