...
If you have an agent that should not interrupt you (e.g. a `review` agent), disable notifications just for that agent:

```markdown
## Agent: review

- Never call the notify tool
- Keep feedback in chat (no system notifications)
```
