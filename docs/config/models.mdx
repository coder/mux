...
## Experimental Image Generation Model

Mux can expose an experimental `image_generate` tool for Exec agents. Enable it in **Settings → Experiments → Image Generation Tool**.

The image model is configured separately from the chat model. The default is `openai:gpt-image-2`; use `openai:gpt-image-2-2026-04-21` if you need a pinned model snapshot. The first experiment is OpenAI-only and requires OpenAI provider credentials in **Settings → Providers**.

The first version is generate-only. It supports prompt, image count, quality, and output format. Generated full-resolution images are saved as runtime-temp artifacts; copy selected final assets into the workspace when they should become project files.
