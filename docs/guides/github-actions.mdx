...
#
# LIMITATIONS:
# - PRs from forks won't be labeled (no access to secrets for security)
# - Only issues and PRs from collaborators/the repo itself are labeled
#
# =============================================================================
...
# Note: Using pull_request (not pull_request_target) for security - secrets
# are not exposed to fork PR content. Fork PRs will be skipped gracefully.
# -----------------------------------------------------------------------------
on:
  issues:
    types: [opened]
      ...
      - name: Select labels with mux
        ...
        run: |
          ...
          if [ -z "$ANTHROPIC_API_KEY" ]; then
            echo "⚠️ ANTHROPIC_API_KEY not available (fork PR?) - skipping auto-label"
