...
# Prompt behavior
prompt:
  append: false # When true, append body to base agent's body (default: replace)
...
**Inheritance behavior:** By default, when an agent has a `base`, the child's body **replaces** the base agent's body. Set `prompt.append: true` to **append** instead—useful for project-local agents that add context without losing the base agent's instructions.
