...
If you prefer to keep using your personal GitHub account but still want clear attribution for AI-generated commits, you can add a `Co-authored-by` trailer using Git's built-in hooks.
...
<Info>
  This approach uses your normal Git identity for commits, but adds a trailer indicating AI involvement.
  The hook only activates inside mux workspaces (detected via `MUX_WORKSPACE_ID`).
</Info>
