...
- **Storybook responsive validation:** Do not prove responsive snapshots by only resizing `iframe.html`; that bypasses Storybook viewport configuration. If a story depends on a breakpoint (wide gutters, mobile, touch), set an explicit story `globals.viewport` for local viewing and validate through the Storybook manager. Add a play/static contract when a missing viewport would silently render the wrong UI. Caveat: the Storybook test-runner (CI `Test / Storybook`) does not apply `globals.viewport`, so plays execute at desktop window size; breakpoint-dependent play assertions must force the narrow width themselves (fixed-width wrapper/decorator, as in `App.phoneViewports.stories.tsx`) or guard on the rendered width before asserting.
