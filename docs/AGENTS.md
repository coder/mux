...
- **Avoid `setTimeout` for component coordination** - using delays to "ensure" another component has processed state is racy and fragile. React effects run after render commits, so child components have already received props by the time parent effects execute. Prefer synchronous effect-based approaches or explicit callbacks.
