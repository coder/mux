...
Toggle **Usage Telemetry** off in **Settings → General**. The change applies immediately (no restart) and persists in `~/.mux/config.json` as `telemetryEnabled: false`.

Alternatively, set `MUX_DISABLE_TELEMETRY` before starting the app:
