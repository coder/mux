...
# Run with positional text, mapped to { "input": "review staged changes" }
mux workflow run echo-review "review staged changes" -e dynamic-workflows

# Run with structured JSON args and machine-readable output
mux workflow run echo-review --args-json '{"base":"main"}' --json -e dynamic-workflows
```
...
| `--runtime <runtime>`       | Runtime selection. Currently only `local` is supported for workflow runs.   |
