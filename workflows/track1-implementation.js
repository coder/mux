...
const CONTEXT = [
  ...
  "- Request path (already log-driven): agentSession.ts sendMessage -> streamWithHistory (re-reads chat.jsonl via historyService.getHistoryFromLatestBoundary; commits partial.json first) -> aiService.streamMessage (system prompt via streamContextBuilder.buildStreamSystemContext) -> messagePipeline.prepareMessagesForProvider (deterministic numbered stages) -> streamManager.streamText. No in-memory transcript cache exists; partials never participate in request builds.",
